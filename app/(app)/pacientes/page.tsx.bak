...
import { pacientesStorage, consultasStorage, antropometriaStorage, bioimpedanciaStorage, examenesStorage, minutasStorage, suplementosStorage, medicamentosStorage, notasStorage } from '@/lib/storage'
...
export default function PacientesPage() {
  ...
  function eliminarPaciente(pacienteId: string, nombrePaciente: string) {
    if (!confirm(`¿Estás seguro que deseas eliminar a ${nombrePaciente}? Esta acción no se puede deshacer.`)) return
    pacientesStorage.delete(pacienteId)
    consultasStorage.getByPaciente(pacienteId).forEach(c => consultasStorage.delete(c.id))
    antropometriaStorage.getByPaciente(pacienteId).forEach(a => antropometriaStorage.delete(a.id))
    bioimpedanciaStorage.getByPaciente(pacienteId).forEach(b => bioimpedanciaStorage.delete(b.id))
    examenesStorage.getByPaciente(pacienteId).forEach(e => examenesStorage.delete(e.id))
    minutasStorage.getByPaciente(pacienteId).forEach(m => minutasStorage.delete(m.id))
    suplementosStorage.getByPaciente(pacienteId).forEach(s => suplementosStorage.delete(s.id))
    medicamentosStorage.getByPaciente(pacienteId).forEach(m => medicamentosStorage.delete(m.id))
    notasStorage.getByPaciente(pacienteId).forEach(n => notasStorage.delete(n.id))
    recargar()
  }
